...
if ! [ -n "${module_samtools}" ]; then
    echo "ERROR: The module SAMTOOLS is not loaded or in the 'PATH'."
    exit 1
elif ! [ -n "${module_gatk}" ]; then
    echo "ERROR: The module GATK is not loaded or in the 'PATH'."
    exit 1
elif ! [ -n "${module_java}" ]; then
    echo "ERROR: The module JAVA/JDK is not loaded or in the 'PATH'."
    exit 1
fi

# Check if needed data and aditional code exist
if ! [ -e "${known_sites}" ]; then
    echo "ERROR: The SV known sites VCF is not in the path specified in this script."
    exit 1
elif ! [ -e "${calculate_statistics}" ]; then
    echo "ERROR: The 'calculate_statistics.sh' file is not in the path specified in this script."
    exit 1
elif ! [ -e "${calculate_depth_coverage}" ]; then
    echo "ERROR: The 'calculate_depth_coverage.sh' file is not in the path specified in this script."
    exit 1
else
    # Set full dir
...
if ! [ -d $TMP_DIR ]; then
    echo -e "\nERROR: Parameter 'TMP_DIR' in 'conf.yaml' is required or set incorrectly.\n"
    exit_abnormal
else
    # Set full dir
...
if ! [ -d $OUTPUT_DIR ]; then
    echo -e "\nERROR: Parameter 'OUTPUT_DIR' is required or set incorrectly.\n"
    exit_abnormal
elif ! [ -w $OUTPUT_DIR ]; then
    echo "The 'OUTPUT' folder does not have write permissions."
    exit_abnormal
else
    # Set full output dir
...
if ! [ -e $REFERENCE_GENOME ]; then
    echo -e "\nERROR: Parameter 'REFERENCE_GENOME' is required or set incorrectly."
    exit_abnormal
else
    # Set full dir
...
if ! [ -e ${dict_ref}.dict ]; then
    echo -e "\nERROR: Missing reference_genome.dict in 'REFERENCE_GENOME' path folder."
    echo "You can run:
              gatk CreateSequenceDictionary -R reference.fasta -O reference.dict"
    exit_abnormal
elif ! [ -e ${REFERENCE_GENOME}.fai ]; then
    echo -e "\nERROR: Missing reference_genome.fasta.fai in 'REFERENCE_GENOME' path folder."
    echo "You can run:
...
if ! [[ ${SORT_THREADS} =~ $re ]] ; then # if $SORT_THREADS is an INT
   echo "ERROR: 'SORT_THREADS' is not a number or INT" 
   exit_abnormal
fi

# SORT_MEM
if ! [[ ${SORT_MEM::-1} =~ $re ]] ; then # if $SORT_MEM is an INT
   echo "ERROR: 'SORT_MEM' is not a number or INT"
   exit_abnormal
elif [[ "${SORT_MEM: -1}" != "K" ]] && [[ "${SORT_MEM: -1}" != "M" ]] && [[ "${SORT_MEM: -1}" != "G" ]]; then
   echo "ERROR: 'SORT_MEM' is not ended with K/M/G."
   exit_abnormal
fi
...
if ! [[ "${BQSR,,}" == "true" ]] && ! [[ "${BQSR,,}" == "false" ]]; then   # if $BQSR is not boolean:
        echo "ERROR: 'BQSR' must be a boolean, true/false."
        exit_abnormal
fi

# FORCE_SORT_COORDINATES
if ! [[ "${FORCE_SORT,,}" == "true" ]] && ! [[ "${FORCE_SORT,,}" == "false" ]]; then   # if $FORCE_SORT is not boolean:
        echo "ERROR: 'FORCE_SORT_COORDINATES' must be a boolean, true/false."
        exit_abnormal
fi
...
if ! [ -e $SAMPLES ]; then
    ...
    if ! [ -s ${OUTPUT_DIR}/.samples_tmp.txt ]; then # Check if the variable is empty
        echo -e "\nERROR: Parameter 'SAMPLES' in 'conf.yaml' has no samples in it or are malformed configure.\n"
    else
        ...
        while read -r line; do
            ...
            if ! test -f "$input_sample"; then
                ...
                then
                    ...
                    if ! [ -z "${header}" ];
                    then
                        echo -e "\n## ERROR: Input file (${input_sample}) does not have a valid header and/or body\n"
                        rm ${OUTPUT_DIR}/.samples_tmp.txt
                        exit_abnormal
                    fi
                else
                    echo -e "## ERROR: Input file (${input_sample}) does not have the proper extension (.bam) or it can't be read properly.\n"
                    rm ${OUTPUT_DIR}/.samples_tmp.txt
                    exit_abnormal
                fi
                   echo -e "## ERROR: Can't start the pipeline for ${input_sample}, input file or path are not correct.\n"
                   rm ${OUTPUT_DIR}/.samples_tmp.txt
                   exit_abnormal
            fi
...
####################
    ...
    do
        ...
        if ! [ -n "${rg_tags}" ]; then
            echo "# There is not Read Group tags (RG tags) in the BAM file, which are required for the next steps. Please add the @RG tags to the BAM file and rerun the script."
            echo "# You can use the next command as an example."
        ...
        if test -f "${INDEX}"; then
            echo "# ${filename}.bam.bai already exists. Skipping the index of the input file."
        else
            if [ "${INDEX_INPUT,,}" = "true" ];
            then
        ...
        then
            ...
            if [ -n "${rg_tags}" ]; then
                ...
                echo "# There is not Read Group tags (RG tags) in the BAM file. A new and automate one will be created for this specific pipeline." > ${enddir}/${filename}.rg_tags
            fi
        ...
        then
            ...
            # SAMTOOLS SORT
            time samtools sort ${fullname} -m ${SORT_MEM} --threads ${SORT_THREADS} -T ${TMP_DIR} -o ${enddir}/${filename}.sort.bam
        ...
        then
            echo "# Start create index for 'sort.marked_duplicates' file" 
        ...
        then
            ...
            gatk BaseRecalibrator -I ${enddir}/${filename}.sort.marked_duplicates.bam -R ${REFERENCE_GENOME} --known-sites ${known_sites} -O ${enddir}/${filename}.recal_data.table1 --tmp-dir ${TMP_DIR} --read-validation-stringency LENIENT --add-output-sam-program-record true 
        ...
        then
            echo "# Start create index for BQSR" 
        ...
        then
            if [[ $N_TMP_FILES -gt 0 ]];
            then
                echo "# Removing ${N_TMP_FILES} temporary file(s)" 
                rm -r ${TMP_DIR}/*
            fi
        fi
